...
class MongoDBDocumentIngestion:
    ...
    async def ingest_pdf(
        self, pdf_path: str, metadata: dict[str, Any] | None = None, clear_existing: bool = False
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            # mongodb-schema-design: Use additive patterns, not blanket deletion
            # Only clear if explicitly requested
            if clear_existing:
                logger.warning("Clearing existing documents before ingestion")
            ...
            return {"status": "error", "error": str(e), "source": pdf_path}
...
class MongoDBRAGGenerator:
    ...
    async def generate_response(
        self, question: str, num_candidates: int = 100, limit: int = 5
    ) -> str:
        """
        ...
        try:
            ...
            documents = await self.ingestion.retrieve_documents(
                query=question, num_candidates=num_candidates, limit=limit, exact=False
            )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        results = await ingestion.retrieve_documents(
            "What are some movies that take place in the ocean?", num_candidates=100, limit=5
        )
