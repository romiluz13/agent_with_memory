...
class AgentConfig:
    ...
    mcp_servers: list[str] | None = None  # MCP server commands
...
class BaseAgent:
    ...
    def __init__(
        self, config: AgentConfig, memory_manager: MemoryManager, checkpointer: Any | None = None
    ):
        """
        ...
        # Agent metadata
        self.agent_id = f"{config.name}_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}"
    ...
    def _build_graph(self) -> StateGraph:
        ...
        workflow.add_conditional_edges(
            ...
            {"tools": "execute_tools", "respond": "generate_response", "end": END},
        )
        ...
        try:
            ...
            if self.tools:
                response = await self.llm.apredict_messages(
                    messages, functions=[tool.as_dict() for tool in self.tools]
                )
    ...
    async def execute_tools_node(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            for tool_call in state["tool_calls"]:
                ...
                tool_input = {"name": tool_call["name"], "args": tool_call["arguments"]}
    ...
    def _create_system_message(self, memory_context: str) -> SystemMessage:
        ...
        base_prompt = (
            self.config.system_prompt or f"You are {self.config.name}, a helpful AI assistant."
        )
    ...
    async def invoke(
        ...
        state: AgentState = {
            ...
            "metadata": {
                ...
                or f"session_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}",
    ...
    async def stream(self, message: str, user_id: str | None = None, session_id: str | None = None):
        """
