...
class WorkingMemory(MemoryStore):
    ...
    async def store(self, memory: Memory) -> str:
        ...
        try:
            ...
            if not memory.ttl:
                memory.ttl = datetime.now(UTC) + timedelta(minutes=60)
    ...
    async def retrieve(
        ...
        try:
            ...
            filter_query = {
                "memory_type": "working",
                "ttl": {"$gt": datetime.now(UTC)},  # Not expired
            }
            if agent_id:
    ...
    async def update(self, memory_id: str, updates: dict[str, Any]) -> bool:
        ...
        try:
            ...
            updates["accessed_at"] = datetime.now(UTC)

            # Extend TTL on update
            if "ttl" not in updates:
                updates["ttl"] = datetime.now(UTC) + timedelta(minutes=30)

            result = await self.collection.update_one(
                {"_id": ObjectId(memory_id)}, {"$set": updates, "$inc": {"access_count": 1}}
            )
    ...
    async def list_memories(
        ...
        try:
            ...
            cursor = cursor.sort([("metadata.priority", -1), ("accessed_at", -1)])
    ...
    async def get_session_context(self, agent_id: str, session_id: str) -> list[Memory]:
        """Get all working memories for a session."""
        return await self.list_memories(
            filters={"agent_id": agent_id, "metadata.session_id": session_id}
        )

    async def clear_session(self, agent_id: str, session_id: str) -> int:
        """Clear all memories for a specific session."""
        result = await self.collection.delete_many(
            {"agent_id": agent_id, "metadata.session_id": session_id, "memory_type": "working"}
        )
        return result.deleted_count

    async def extend_ttl(self, memory_id: str, minutes: int = 30) -> bool:
        """Extend the TTL of a working memory."""
        new_ttl = datetime.now(UTC) + timedelta(minutes=minutes)
        return await self.update(memory_id, {"ttl": new_ttl})

    async def cleanup_expired(self) -> int:
        """Remove expired working memories."""
        result = await self.collection.delete_many(
            {"memory_type": "working", "ttl": {"$lt": datetime.now(UTC)}}
        )
